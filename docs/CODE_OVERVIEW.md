...
Aturan ketat: **tidak boleh ubah schema `IncidentState` tanpa alasan kuat** (tercantum di `pedoman proyek`).
...
**Masalah:** Per pedoman proyek rule, **JSON parsing WAJIB error handling**. Namun di tiga service, `json.loads()` tidak di-wrap try/except:
